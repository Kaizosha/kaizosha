...
The system should make pages feel related without making every product look
identical. Shared structure belongs in `assets/styles/site-system.css`.
Product color, diagrams, and interaction belong in each product stylesheet.
...
2. **One shell, distinct products.** Headers, legal navigation, footers,
   spacing rhythm, and focus behavior are shared. Each product may own one
   signal color and its primary visual system.
3. **Content sets the scale.** Large type belongs to product or identity
...
5. **Motion explains state.** Animation may communicate opening, progress,
   selection, or a product concept. It must not delay navigation and must
   respect `prefers-reduced-motion`.
6. **Plain language wins.** Product promises, legal text, status, and actions
...
| `product` | Sekai, Together, TL;Pinch, future products | Standard product header, one literal H1, numbered sections, closing statement, legal footer |
...
| `immersive` | `i` and future narrative experiences | Full-bleed experience; retain skip link, H1, reduced motion, and brand metadata |
...
- Product and identity H1s may be large; never use that scale inside controls,
...
- A product may define one `--page-signal` color for product state, focus, and
  diagram emphasis.
...
Use only the lockups defined in `BRAND.md`. Product headers use the full lockup
on larger screens and collapse to the compact lockup below 640px.

### Product Header

- Brand on the left; page navigation or product status on the right.
- Use the shared header height and shell gutter.
- Keep labels on one line when possible; hide secondary navigation before it
  collides with the brand.
- The product status is text plus a small signal indicator, not a badge.
...
- End directory, product, and identity pages with `FROM ME COMES THE FUTURE`
...
- Order: Creator when relevant, Terms, Privacy, Contact.
...
- Product heroes collapse to one column before copy or media becomes cramped.
...
4. Define `--page-signal` only when the page is a product.
